...
export default function ManageScreen() {
  ...
  const [pickerTarget, setPickerTarget] = useState(null); // 'mp' | 'poi' | null
    ...
    return (
      <View style={styles.emptyContainer}>
        ...
        <Text style={styles.emptySubtitle}>
          Go to Home and select a tour to manage
        </Text>
      </View>
    );
  }
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
            ...
            return (
              <Pressable
                ...
                style={[
                  styles.statusChip,
                  {
                    backgroundColor: active ? theme.bg : COLORS.bg,
                    borderColor: active ? theme.text + "40" : COLORS.border,
                  },
                ]}
              ...
              >
                <Text
                  style={[
                    ...
                    { color: active ? theme.text : COLORS.textMuted },
                  ]}
                >
                  {theme.label}
                </Text>
      ...
      {pickerTarget && (
        <View style={[styles.card, SHADOWS.small]}>
          <Text style={styles.cardTitle}>
            Tap the map to set {pickerTarget === "mp" ? "meeting point" : "POI"}{" "}
            location
          </Text>
          <MapView
            ref={miniMapRef}
      ...
      <View style={[styles.card, SHADOWS.small]}>
        ...
        <PlaceSearchInput
          onPlaceSelected={handleMpPlaceSelected}
          placeholder="Search for a meeting place..."
        />
        <TextInput
          style={styles.input}
          value={mpName}
          onChangeText={setMpName}
          placeholder="Name"
        ...
        <TextInput
          style={styles.input}
          value={mpTime}
          onChangeText={setMpTime}
          placeholder="Meetup time (optional, e.g. 14:30)"
          placeholderTextColor={COLORS.textMuted}
        />
        <Pressable
          style={styles.toggleRow}
          onPress={() => setMpCurrent((v) => !v)}
        >
          <Ionicons
            name={mpCurrent ? "checkbox" : "square-outline"}
            size={20}
        ...
        {mps.map((mp) => (
          <View key={mp.id} style={styles.listItem}>
            <View style={{ flex: 1 }}>
              ...
              {mp.meetupTime && (
                <Text style={styles.listMeta}>Time: {mp.meetupTime}</Text>
              )}
            </View>
            <Pressable
              style={styles.deleteBtn}
              onPress={() => wrap(() => deleteMeetingPoint(mp.id))}
            >
      ...
      <View style={[styles.card, SHADOWS.small]}>
        ...
        <PlaceSearchInput
          onPlaceSelected={handlePoiPlaceSelected}
        ...
        {pois.map((poi) => (
          <View key={poi.id} style={styles.listItem}>
            <View style={{ flex: 1 }}>
              ...
              {poi.description && (
                <Text style={styles.listMeta}>{poi.description}</Text>
              )}
            </View>
            <Pressable
              style={styles.deleteBtn}
              onPress={() => wrap(() => deletePoi(poi.id))}
            >
              <Ionicons name="trash-outline" size={18} color={COLORS.danger} />
            </Pressable>
          </View>
...
const styles = StyleSheet.create({
  ...
  miniMap: { width: "100%", height: 200, borderRadius: 12 },
  cancelPickerBtn: { alignItems: "center", paddingVertical: 8 },
  cancelPickerText: { color: COLORS.danger, fontWeight: "700" },
  input: {
    borderWidth: 1,
