...
export default function MapScreen() {
  ...
  useEffect(() => {
    ...
    return () => { sub?.remove(); watchRef.current = null; };
  }, [setMyLocation]);
  ...
  if (!initialRegionRef.current) {
    const src = lastGuideLocation || myLocation || currentMeetingPoint || pois[0];
    initialRegionRef.current = src
      ? { latitude: src.lat, longitude: src.lng, latitudeDelta: 0.025, longitudeDelta: 0.025 }
      : DEFAULT_REGION;
  }
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        {lastGuideLocation && (
          <Marker
            coordinate={{ latitude: lastGuideLocation.lat, longitude: lastGuideLocation.lng }}
          ...
          >
            <View style={[
              styles.guideDot,
              (isGuideLocationStale || lastGuideLocation.isFallback) && styles.guideDotStale,
            ]}>
      ...
      {showLegend && (
        <View style={[styles.legendPanel, SHADOWS.medium]}>
          ...
          <LegendRow color="#4A90D9" label="Your location (blue dot)" icon="ellipse" />
          <LegendRow color="#16A34A" label="Guide (live)" icon="navigate" />
          <LegendRow color="#F59E0B" label="Guide (stale/last known)" icon="navigate" />
